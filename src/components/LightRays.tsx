...
const LightRays: React.FC<LightRaysProps> = ({
  ...
  useEffect(() => {
    ...
    const initializeWebGL = async () => {
      ...
      // Responsieve DPR (lagere DPR op kleine schermen voor FPS)
      const isSmall = window.innerWidth < 640
      const targetDpr = Math.min(window.devicePixelRatio || 1, isSmall ? 1.25 : 2)

      const renderer = new Renderer({ dpr: targetDpr, alpha: true })
      ...
      const updatePlacement = () => {
        ...
        const isSmallNow = window.innerWidth < 640
        renderer.dpr = Math.min(window.devicePixelRatio || 1, isSmallNow ? 1.25 : 2)
      ...
      const loop = (t: number) => {
        ...
        if (!prefersReduced && followMouse && mouseInfluence > 0) {
    ...
    noiseAmount, distortion, prefersReduced
  ])
